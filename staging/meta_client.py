...
def offsite_leads_of(row: dict) -> int:
    return _action_value(row, ["offsite_conversion.fb_pixel_lead"])
...
class MetaClient:
            ...
            try:
                ...
                if "error" in data:
                    ...
                    if code in (17, 32, 4, 613) or err.get("is_transient"):
                        ...
                        log.warning(f"Rate limit (code {code}), waiting {wait}s")
                        time.sleep(wait)
                        continue
    ...
    def list_ads(self, campaign_id: str) -> list[dict]:
        return self._paginate(f"{campaign_id}/ads", {"fields": AD_FIELDS, "limit": 500})
    ...
    def get_creatives_by_ids(self, creative_ids: list[str]) -> dict[str, dict]:
        ...
        def _fetch_chunk(chunk: list[str]) -> None:
            try:
                ...
                if len(chunk) > 1:
                    mid = len(chunk) // 2
                    log.warning(f"  creatives batch of {len(chunk)} failed; splitting")
                    _fetch_chunk(chunk[:mid])
                    _fetch_chunk(chunk[mid:])
                else:
                    log.warning(f"  creative {chunk[0]} unfetchable: {e}")

        chunk_size = 10
        for i in range(0, len(creative_ids), chunk_size):
            _fetch_chunk(creative_ids[i : i + chunk_size])
